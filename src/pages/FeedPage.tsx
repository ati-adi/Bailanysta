...
export default function FeedPage() {
  const navigate = useNavigate()
  const { posts, currentUser, followedIds, hasNewPosts, markFeedSeen, pushToast } = useApp()
  const [filter, setFilter] = useState<FeedFilter>('all')
  const [animateKey, setAnimateKey] = useState<string>('initial')
  const [spinning, setSpinning] = useState(false)
  const [scrolledDown, setScrolledDown] = useState(false)

  // Баннер «новые посты» — только когда пользователь прокрутил ленту вниз
  useEffect(() => {
    const onScroll = () => setScrolledDown(window.scrollY > 240)
    window.addEventListener('scroll', onScroll, { passive: true })
    return () => window.removeEventListener('scroll', onScroll)
  }, [])
  ...
  return (
    <div>
      ...
      <NewPostsBanner visible={hasNewPosts && scrolledDown} onClick={handleShowNew} />
