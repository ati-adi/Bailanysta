...
export default function WhoToFollow() {
  const { users, usersLoading, currentUser, toggleFollow, pushToast } = useApp()
  ...
  return (
    ...
    >
      ...
      {usersLoading ? (
        <div className="flex flex-col gap-3 px-2 py-2" role="status" aria-label="Загрузка рекомендаций">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex items-center gap-3">
              <div className="h-10 w-10 shrink-0 animate-pulse rounded-full bg-line" />
              <div className="min-w-0 flex-1">
                <div className="h-3.5 w-2/3 animate-pulse rounded-full bg-line" />
                <div className="mt-1.5 h-3 w-1/3 animate-pulse rounded-full bg-line" />
              </div>
            </div>
          ))}
        </div>
